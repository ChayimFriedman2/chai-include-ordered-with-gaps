...
module.exports = function(chai, utils) {
  ...
  assert.includeOrderedMembersWithGaps = function(superset, subset) {
    new Assertion(superset).to.include.ordered.members.with.gaps(subset);
  }

  assert.includeDeepOrderedMembersWithGaps = function(superset, subset) {
    new Assertion(superset).to.include.deep.ordered.members.with.gaps(subset);
  }

  assert.notIncludeOrderedMembersWithGaps = function(superset, subset) {
    new Assertion(superset).to.not.include.ordered.members.with.gaps(subset);
  }

  assert.notIncludeDeepOrderedMembersWithGaps = function(superset, subset) {
    new Assertion(superset).to.not.include.deep.ordered.members.with.gaps(subset);
  }
};
